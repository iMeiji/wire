...
class BindingGenerator private constructor(
  ...
  private val Service.serviceName
    get() = type.typeName
  private val Type.pbPackageName
    get() = schema.protoFile(type)!!.javaPackage()!!
              ...
              when (fieldOrOneOf) {
                is Field -> {
                  if (fieldOrOneOf.name == fieldOrOneOf.type!!.simpleName ||
                    ...
                    newName(fieldOrOneOf.compatibleGooglePbFieldName().snakeToLowerCamelCase(), fieldOrOneOf)
                  }
                }
                is OneOf -> {
                  ...
                  newName(
                    boxedOneOfClassName(fieldOrOneOf.name),
                    boxedOneOfClassName(fieldOrOneOf.name)
                  )
...
//    }
  ...
  private fun addConvertMethod(
    ...
    val pbPackageName = type.pbPackageName
  ...
  //  private fun Field.getConvertCodeBlock(nameAllocator: NameAllocator): CodeBlock {
//    val fieldName = nameAllocator[this]
//    val itemType = type!!.typeName
//    if (isRepeated) {
//      if (isScalar) {
//        CodeBlock.of("this.%1N = pb.%2N.mapNotNull{it}", fieldName, fieldName)
//      } else {
//        CodeBlock.of("this.%1N = pb.%2N.mapNotNull{%3T.convert(it)}\n",
//          fieldName,
//          fieldName,
//          itemType)
//      }
    ...
    val returnType = when (field.label) {
      Field.Label.REPEATED -> when {
        field.type!!.isScalar -> when (field.type!!.typeName) {
          ...
          String::class.asClassName() -> Array::class.asClassName()
            .parameterizedBy(field.type!!.typeName)
          else -> throw IllegalStateException("Unsupported annotation for ${field.type}")
        }
        schema.getType(field.type!!) is EnumType -> Array::class.asClassName()
          .parameterizedBy(field.type!!.typeName)
        else -> throw IllegalStateException("Unsupported annotation for ${field.type}")
      }
  ...
  companion object {
    ...
    operator fun invoke(
      ...
      fun putAll(kotlinPackage: String, enclosingClassName: ClassName?, types: List<Type>) {
        for (type in types) {
          val className = enclosingClassName?.nestedClass(type.type.simpleName + nameSuffix)
            ?: ClassName(kotlinPackage, type.type.simpleName + nameSuffix)
      ...
      for (protoFile in schema.protoFiles) {
        val kotlinPackage = javaPackage(protoFile).lowercase(Locale.getDefault())
    ...
    private val Extend.annotationTargets: List<AnnotationTarget>
      get() = when (type) {
        ...
        FIELD_OPTIONS, ENUM_VALUE_OPTIONS -> listOf(
